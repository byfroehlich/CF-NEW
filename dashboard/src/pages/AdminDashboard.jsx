...
import { logout, getJobs, getJobSummary, getCreators, getAgencies, createCreator, createAgency, getLogs, getLogSummary, updateCreatorPhoto } from '../lib/api.js'
      ...
      <div className="flex items-center justify-between max-w-6xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div className="pl-2 border-l border-white/20">
            <div className="text-xs text-white/50 uppercase tracking-widest font-semibold">Admin</div>
          </div>
        </div>
            ...
            <div key={c.id} className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  {agencyName && <p className="text-xs text-gray-400 mt-0.5">{agencyName}</p>}
                </div>
              </div>
              ...
              <div className="space-y-1 text-xs text-gray-500">
                ...
                {age !== null && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>{age} Jahre</span>
                    {age < 18 ? <span className="text-red-600 font-semibold">⚠ Minderjährig</span> : <span className="text-green-600">✓ 18+</span>}
                  </div>
                )}
                <div className="flex items-center gap-1.5">
...
function SystemTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {['Alle Level','Info','Warn','Error'].map(l => <button key={l} onClick={() => setLevel(l)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${level===l ? 'bg-violet-700 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{l}</button>)}
